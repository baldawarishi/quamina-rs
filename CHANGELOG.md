...
### Fixed
- `matcher_stats()` and `arena_stats()` now count the lazy DFA cache, which a `BuiltForSpeed` pattern falls back to when its DFA is too costly to build up front. The cache holds a second copy of the pattern's NFA and grows a 256-entry table per DFA state as events arrive, and none of it was reported: the two modes came back byte-for-byte identical for such a pattern, however long it had been matching (#185)
- Lookbehind and lookahead assertions in one regexp now agree on what the value looks like, so `(?<=foo)bar(?=baz)baz` matches `foobarbaz`. Each was built as if it were the only assertion: the lookahead's condition left out the run the lookbehind accounts for ahead of the primary, and the lookbehind's left out the run a closing lookahead accounts for past it, so a pattern mixing the two matched nothing at all (#184)
- A lookaround assertion that the pattern continues past now only has to account for the front of the value, leaving the rest to the atoms that follow it. Previously every assertion was checked against the whole value, so `(?=.*foo).*bar.*` matched nothing at all — not even `foobar` (#183)
