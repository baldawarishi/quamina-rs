...
fn test_bad_pattern_error_handling() {
    ...
    // Empty anything-but
    assert!(
        q.add_pattern("p1", r#"{"x": [{"anything-but": []}]}"#)
            .is_err()
    );

    // Mixed types in anything-but
    assert!(
        q.add_pattern("p2", r#"{"x": [{"anything-but": ["a", 1]}]}"#)
            .is_err()
    );
}
...
fn test_default_limits_allow_normal_patterns() {
    ...
    assert!(
        q.add_pattern("prefix", r#"{"x": [{"prefix": "he"}]}"#)
            .is_ok()
    );
    assert!(
        q.add_pattern("suffix", r#"{"x": [{"suffix": "lo"}]}"#)
            .is_ok()
    );
    assert!(
        q.add_pattern("shell", r#"{"x": [{"shellstyle": "h*o"}]}"#)
            .is_ok()
    );
    assert!(
        q.add_pattern("wild", r#"{"x": [{"wildcard": "h*o"}]}"#)
            .is_ok()
    );
    assert!(
        q.add_pattern("ab", r#"{"x": [{"anything-but": ["no"]}]}"#)
            .is_ok()
    );
    assert!(
        q.add_pattern("eic", r#"{"x": [{"equals-ignore-case": "HELLO"}]}"#)
            .is_ok()
    );
    assert!(
        q.add_pattern("re", r#"{"x": [{"regex": "[a-z]+"}]}"#)
            .is_ok()
    );
    assert!(
        q.add_pattern("numr", r#"{"x": [{"numeric": [">=", 1, "<", 100]}]}"#)
            .is_ok()
    );
    assert!(
        q.add_pattern("cidr", r#"{"x": [{"cidr": "10.0.0.0/8"}]}"#)
            .is_ok()
    );
    assert!(
        q.add_pattern("exists", r#"{"x": [{"exists": true}]}"#)
            .is_ok()
    );
}
...
fn test_state_limit_default_allows_normal_patterns() {
    ...
    assert!(
        q.add_pattern("p1", r#"{"status": ["active", {"prefix": "pend"}]}"#)
            .is_ok()
    );

    // Multiple fields with single matchers (no multiplication)
    assert!(
        q.add_pattern("p2", r#"{"a": ["1"], "b": ["2"], "c": ["3"]}"#)
            .is_ok()
    );
...
fn test_numeric_comparison_invalid_patterns() {
    ...
    // Unknown operator
    assert!(
        q.add_pattern("bad", r#"{"x": [{"numeric": ["!=", 5]}]}"#)
            .is_err()
    );

    // Non-number value after operator
    assert!(
        q.add_pattern("bad", r#"{"x": [{"numeric": [">", "five"]}]}"#)
            .is_err()
    );

    // Non-string in operator position (number where operator expected)
    assert!(
        q.add_pattern("bad", r#"{"x": [{"numeric": [5, 10]}]}"#)
            .is_err()
    );
}
...
fn test_validate_wildcard_escapes() {
    ...
    // Valid escaped star
    assert!(
        q.add_pattern("esc", r#"{"x": [{"wildcard": "a\\*b"}]}"#)
            .is_ok()
    );

    // Trailing backslash — invalid
    assert!(
        q.add_pattern("bad", r#"{"x": [{"wildcard": "a\\"}]}"#)
            .is_err()
    );

    // Invalid escape character (not * or \)
    assert!(
        q.add_pattern("bad2", r#"{"x": [{"wildcard": "a\\nb"}]}"#)
            .is_err()
    );

    // Adjacent ** in wildcard — invalid
    assert!(
        q.add_pattern("bad3", r#"{"x": [{"wildcard": "a**b"}]}"#)
            .is_err()
    );
}
