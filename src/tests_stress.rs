...
// ============================================================================
// Shell-style build time stress test
// ============================================================================

/// Port of Go's TestShellStyleBuildTime from shell_style_test.go.
///
...
/// 1000 onto a single field, then verifies that every original word and every
/// "expanded" word (with ÉÉÉÉ inserted at the star position) matches.
///
...
// MIRI SKIP RATIONALE: Adds 1000 shellstyle patterns and matches 2000 events.
// Heavy NFA traversal would take hours under Miri interpretation.
#[test]
#[cfg_attr(miri, ignore)]
fn test_shellstyle_build_time() {
    use rand::{RngExt, SeedableRng};
    use std::fs;
    use std::path::Path;
    ...
    if !wwords_path.exists() {
        eprintln!("Skipping test_shellstyle_build_time: testdata/wwords.txt not found");
        return;
    }
    ...
    let words: Vec<&str> = contents
        ...
        .take(1000)
        .collect();
    assert_eq!(words.len(), 1000, "Need at least 1000 words in wwords.txt");

    // Use a deterministic RNG (seed matches Go's 293591 conceptually,
    // though the actual sequence differs between Go's math/rand and Rust's StdRng)
    ...
    for word in &words {
        ...
        // Clamp to word length to avoid panics on short words
    ...
    for (i, pattern) in patterns.iter().enumerate() {
        q.add_pattern(star_words[i].clone(), pattern)
            .unwrap_or_else(|e| panic!("AddPattern failed for '{}': {}", star_words[i], e));
    }

    // Verify all original words match
    for (i, word) in words.iter().enumerate() {
        let event = format!(r#"{{"x": "{}"}}"#, word);
        let matches = q.matches_for_event(event.as_bytes()).unwrap();
        assert!(
            !matches.is_empty(),
            "No matches for original word '{}' (pattern '{}')",
            word,
            star_words[i]
        );
    }

    // Verify all expanded words match
    for (i, expanded) in expanded_words.iter().enumerate() {
        let event = format!(r#"{{"x": "{}"}}"#, expanded);
        let matches = q.matches_for_event(event.as_bytes()).unwrap();
        assert!(
            !matches.is_empty(),
            "No matches for expanded word '{}' (pattern '{}')",
            expanded,
            star_words[i]
        );
    }
