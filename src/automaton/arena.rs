...
mod kani_arena_proofs {
    ...
    /// Prove: pack() followed by dstep() is a faithful roundtrip for any
    /// 3-region unpacked table and any lookup byte.
    ///
    /// Constructs a BYTE_CEILING-entry unpacked array with 3 symbolic
    /// region boundaries and state IDs, calls pack(), then verifies that
    /// dstep(byte) == unpacked[byte] for a symbolic byte.
    ///
    /// The loop bound is BYTE_CEILING+1 (247) because pack iterates over
    /// all 246 entries. Limited to 3 regions to keep the proof tractable.
    #[kani::proof]
    #[kani::unwind(247)]
    fn smalltable_pack_dstep_roundtrip() {
        // Choose 3 symbolic region boundaries: 0 < b0 < b1 <= BYTE_CEILING
        let b0: u8 = kani::any();
        let b1: u8 = kani::any();
        kani::assume(b0 > 0);
        kani::assume(b1 > b0);
        kani::assume((b1 as usize) <= BYTE_CEILING);

        // Choose 3 symbolic state IDs for the 3 regions
        let s0 = StateId::from_index(kani::any::<u8>() as usize);
        let s1 = StateId::from_index(kani::any::<u8>() as usize);
        let s2 = StateId::from_index(kani::any::<u8>() as usize);

        // Build the unpacked table: region [0, b0) = s0, [b0, b1) = s1, [b1, BYTE_CEILING) = s2
        let mut unpacked = [StateId::NONE; BYTE_CEILING];
        let mut i: usize = 0;
        while i < BYTE_CEILING {
            if i < b0 as usize {
                unpacked[i] = s0;
            } else if i < b1 as usize {
                unpacked[i] = s1;
            } else {
                unpacked[i] = s2;
            }
        ...
        // Verify roundtrip for a symbolic byte
        let byte: u8 = kani::any();
        kani::assume((byte as usize) < BYTE_CEILING);

        let result = table.dstep(byte);
        kani::assert(
            result == unpacked[byte as usize],
            "pack then dstep must return the original unpacked value",
        );
    }
}
